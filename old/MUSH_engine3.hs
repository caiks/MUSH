...
main = 
  do
    ...
    (uu,hh,hht) <- do
      ...
      let uu = sys aa
      let xx = sort $ zip (randoms (mkStdGen 100) :: [Double]) (aall aa)
      let aa1 = llaa $ map snd $ drop 812 xx
      let aa2 = llaa $ map snd $ take 812 xx
      return (uu, aahr uu aa1, aahr uu aa2)
    ...
    printf "history non-modal size: %d\n" $ numerator (size aa - amax aa)

    let model = "MUSH_model3"
    let (wmax,lmax,xmax,omax,bmax,mmax,umax,pmax,fmax,mult,seed) = ((9*9*10), 8, (9*9*10), 10, (10*3), 3, (9*9*10), 1, 30, 1, 1)

    printf ">>> %s\n" $ model
    Just (uu',df) <- decomperIO uu vvk hh wmax lmax xmax omax bmax mmax umax pmax fmax mult seed
    ...
    printf "label non-modal size: %d\n" $ numerator $ sum $ map (\aa -> size aa - amax aa) $ systemsDecompFudsHistoryRepasSetVariablesListHistogramLeaf uu' df hh vvl
