...
main = 
  do
    ...
    let model = "MUSH_model6"
    let (wmax,lmax,xmax,omax,bmax,mmax,umax,pmax,fmax,mult,seed) = ((9*9*10), 8, (9*9*10), 10, (10*3), 3, (9*9*10), 1, 2, 1, 1)

    printf ">>> %s\n" $ model
    Just (uu',df) <- decomperIO uu vvk aa wmax lmax xmax omax bmax mmax umax pmax fmax mult seed
    ...
    printf ">>> %s\n" $ model
    let Just (uu',df2) = decomper uu vvk aa wmax lmax xmax omax bmax mmax umax pmax fmax mult seed
    printf "<<< done %s\n" $ model
    hFlush stdout
    printf "decomp size: %d\n" $ (Set.size $ fvars $ dfff df2)
    hFlush stdout

    printf ("decomps equal: " ++ (if (df2 == df) then "true" else "false") ++ "\n")
    hFlush stdout
  ...
  where 
    ...
    decomper uu vv hh wmax lmax xmax omax bmax mmax umax pmax fmax mult seed =
